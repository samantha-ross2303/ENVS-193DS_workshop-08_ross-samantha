...
drought_exp_clean <- drought_exp |> # starting with the drought_exp object
  ...
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # making sure water_treatment is a factor and reordering the levels
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As SLA increases, total plant mass increases") # notice how it has a theme different from default
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "average total mass is higher in well watered treatment \n (and spread is higher than in drought stressed)") +
  # taking out legend
  theme(legend.position = "none") # /n adds a line break so it all fits
```
...
# model 0: null model
model0 <- lm(
  total_g ~ 1, # formula (1 represents no predictors)
